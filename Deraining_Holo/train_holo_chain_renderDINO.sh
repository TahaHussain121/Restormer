...
# render, cropped and flipped identically to the LQ).
#
# Identical mechanism to train_holo_chain_verynoisy.sh, but targets the separate
# experiment "Holo_DINOv2_renderDINO_verynoisy" (clean GT + verynoisy LQ, mixup
# off, RestormerDINO + frozen DINOv2 FiLM with centered pooled features). Its own
...
START_TS=$(date +%s)
python basicsr/train.py -opt Deraining_Holo/Options/Holo_DINOv2_renderDINO_Restormer.yml --launcher none
TRAIN_RC=$?
END_TS=$(date +%s)
