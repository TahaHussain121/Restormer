"""aca-L36: ACA fusion over the DINO layer set {3,6}.
...
THE PIPELINE, replacing exactly one tensor at `inp_enc_level4`:
    ...
    for l in {3,6}:  D_l = centered DINOv2 patch grid  [B,768,g,g]
...
uniform at 1/2 = 0.5000. `project_out` zero -> step-0 output
...
LAYERS_1INDEXED = [3, 6]          # HARD-CODED. Not a config flag.
REFERENCE_BLOCK = 6                 # must be in the set; ties the arm to B6


class RestormerAcaL36Render(RestormerDinoSpatialRender):

    def __init__(self, *args, **kwargs):
        fusion = kwargs.pop('dino_fusion', 'aca')
        if fusion != 'aca':
            raise ValueError(f'RestormerAcaL36Render requires dino_fusion: aca, got {fusion!r}')
        if 'dino_layers' in kwargs:
            raise ValueError(
                'aca-L36 hard-codes its layer set [3, 6]; dino_layers is not a '
    ...
    def dino_prior(self, inp_img, mode, latent_hw):
        """render -> AFFM-fused centered grid [B,768,g,g] over {3,6}."""
